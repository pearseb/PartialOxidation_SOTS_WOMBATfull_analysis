...
for nn in $(seq 1 1 100); do
  wrkdir="/g/data/p93/pjb581/experiments/"
  cd $wrkdir
  cp -r om3_mom6_1D-SOTSfull_opt om3_mom6_1D-SOTSfull_optimal${nn}/
done

for nn in $(seq 1 1 100); do
  cd $wrkdir
  cd om3_mom6_1D-SOTSfull_optimal${nn}/
  cp ../om3_mom6_1D-SOTSfull_opt/field_table ./

  nrow=$((nn+1))
  param_file="/g/data/vn19/pjb581/SOTS-Optimization/WOMBATfull/data/parameter_sets_optimal100.txt"
...
for nn in $(seq 1 1 100); do
  cd $wrkdir
  cp om3_mom6_1D-SOTSfull_optimal${nn}/field_table tmp_field_table/field_table_${nn}
done
...
for nn in $(seq 1 1 100); do
  cd $wrkdir
  cd om3_mom6_1D-SOTSfull_optimal${nn}
  sed -i "s/jobname: SOTSfull/jobname: optimal${nn}/" config.yaml
